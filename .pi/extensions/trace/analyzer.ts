...
export interface RunSummary {
	...
	duration_ms: number;
	specs: SpecSummary[];
	totals: { tokens_in: number; tokens_out: number; tool_calls: number };
}

export function buildSummary(tracesDir: string, rootSessionId: string): RunSummary | null {
	const entries = deduplicateIndex(readIndex(tracesDir));
	const entryMap = new Map(entries.map((e) => [e.session_id, e]));
	...
	// Group children by spec
	const specMap = new Map<string, { phase: string; agent: string; childId: string }[]>();
	for (const link of childLinks) {
		const ctx = link.data.context as Record<string, unknown> | undefined;
		const spec = (ctx?.spec as string) || "(no spec)";
		const phase = (ctx?.phase as string) || "unknown";
		const agent = (link.data.agent as string) || "unknown";
		const childId = link.data.child_session as string;

		if (!specMap.has(spec)) specMap.set(spec, []);
		specMap.get(spec)!.push({ phase, agent, childId });
	}
	...
	for (const [spec, children] of specMap) {
		const phases: SpecPhaseEntry[] = [];
		let specDuration = 0;
		let specTokensIn = 0;
		let specTokensOut = 0;
		...
		for (const child of children) {
			const entry = entryMap.get(child.childId);
			const duration = entry?.duration_ms ?? 0;
			const tokens = entry?.tokens ?? { in: 0, out: 0 };
			const outcome = entry?.outcome ?? "unknown";
			const toolCalls = entry?.tool_calls ?? 0;
			...
			specTokensIn += tokens.in;
			specTokensOut += tokens.out;
			totalTokensIn += tokens.in;
			totalTokensOut += tokens.out;
			totalToolCalls += toolCalls;

			if (outcome === "error") lastOutcome = "error";
		}

		specs.push({
			spec,
			phases,
			outcome: lastOutcome === "error" ? "blocked" : "completed",
			total_duration_ms: specDuration,
			total_tokens: { in: specTokensIn, out: specTokensOut },
		});
	}
	...
	return {
		...
		duration_ms: rootEntry.duration_ms,
		specs,
		totals: { tokens_in: totalTokensIn, tokens_out: totalTokensOut, tool_calls: totalToolCalls },
	};
}
...
export function filterChildren(tracesDir: string, rootSessionId: string, filters: ChildFilter): FilteredChild[] {
	const entries = deduplicateIndex(readIndex(tracesDir));
	const entryMap = new Map(entries.map((e) => [e.session_id, e]));
	...
	for (const link of childLinks) {
		...
		if (filters.phase && !phase.includes(filters.phase)) continue;
		if (filters.spec && !spec.includes(filters.spec)) continue;

		const entry = entryMap.get(childId);
		results.push({
			session_id: childId,
			agent: (link.data.agent as string) || "unknown",
			phase,
			...
			tokens: entry?.tokens ?? { in: 0, out: 0 },
		});
	}
