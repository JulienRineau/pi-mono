			...
			switch (params.action) {
				...
				case "read": {
					...
					if (params.event_type) {
						...
						if (events.length === 0) {
							return textResult(`No events of type "${params.event_type}" found`);
						}
					}
				...
				case "title": {
					...
					return textResult(ok ? `Title updated for ${params.session_id}` : `Session not found: ${params.session_id}`);
				}
